...
class ExperimentalDesign:
    ...
    def __init__(self, csv_path="dissertation-experiment-data.csv", resource_friendly=True):
        ...
        self.batch_delay = 30.0 if resource_friendly else 10.0     # Long delay between batches
    ...
    def get_experimental_dataset(self, use_full_dataset=True, size=30, strategy='balanced', random_state=42):
        """
        Get experimental dataset from real data using specified strategy.
        
        Args:
            use_full_dataset (bool): If True, use the entire dataset; if False, use sampling
            size (int): Number of samples to use (only when use_full_dataset=False)
            strategy (str): Sampling strategy ('balanced', 'stratified', 'random') (only when use_full_dataset=False)
        ...
        """
        if use_full_dataset:
            print(f"🧪 Using entire dataset for experimental analysis")
            dataset = self.dataset_loader.get_full_dataset()
        else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        designer.create_experiment(
            ...
            description="Performance analysis using entire SetFit/toxic_conversations dataset",
            parameters={
                "dataset_source": "SetFit/toxic_conversations",
                "use_full_dataset": True,
                "sampling_strategy": "none (full dataset)",
                "resource_friendly": True
            }
        )
        
        # Generate test dataset from real data - use entire dataset
        dataset = designer.get_experimental_dataset(use_full_dataset=True)
